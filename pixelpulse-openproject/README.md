...
After a while, OpenProject should be up and running on `http://umbrel.local:7654`. The default username and password is login: `admin`, and password: `admin`. 
...
If you don't want OpenProject to bind to `0.0.0.0` you can bind it to umbrel.local only like this:

```
PORT=192.168.7.206:7654
```
